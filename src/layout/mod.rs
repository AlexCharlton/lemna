...
impl super::node::Node {
    ...
    fn resolve_child_sizes(
        ...
        let available_size = size
            ...
            .min(max_size)
            .minus_bounds(&padding);
        let assigned_size = self.layout_result.size.min(max_size).minus_bounds(&padding);
        ...
        if cfg!(debug_assertions) && self.resolved_layout.debug.is_some() {
            log_debug!(
                "resolve_child_sizes: <{}> in bounds {:?}, inner_size: {:?}, available_size: {:?}, assigned_size: {:?}",
                self.resolved_layout.debug.as_ref().unwrap(),
                &bounds_size,
                &inner_size,
                &available_size,
        ...
        let max_available = if bounds_size.main(dir).resolved() {
            // Always use available or bounds_size as the constraint from the parent, not inner_size
            // which might be from a previous pass and exceed the parent's constraint
            available_size
                .main(dir)
                .most_specific(&bounds_less_padding.main(dir))
        ...
        // Maximum available space for the main axis
        let mut main_remaining = f64::from(max_available);
        // Available space using already allocated size
        let mut self_main_remaining = f64::from(assigned_size.main(dir).maybe_px().unwrap_or(0.0));
        ...
        for child in self.children.iter_mut() {
            ...
            let mut width_pct = pre_resolved_size.width.is_pct();
            let mut height_pct = pre_resolved_size.height.is_pct();
            ...
            if self.resolved_layout.cross_alignment.unwrap() == Alignment::Stretch {
                *resolved_size.cross_mut(dir) =
                    Dimension::Pct(100.0).maybe_resolve(&assigned_size.cross(dir));
                if self.resolved_layout.direction.unwrap() == Direction::Row {
                    height_pct = true;
                } else {
                    width_pct = true;
                }
                ...
                && child.layout_result.main_resolved
            {
                if !self.resolved_layout.wrap && !child.resolved_layout.overlay {
                    ...
                    self_main_remaining -= x + f64::from(child_margin.main_total(dir));
                }
            } else {
                unresolved += 1;
        ...
        self_main_remaining = self_main_remaining.max(0.0);
        ...
        for child in self.children.iter_mut() {
            ...
            if self.resolved_layout.axis_alignment.unwrap() == Alignment::Stretch
                && self_main_remaining > 0.0
                && !child.layout_result.size.main(dir).resolved()
                && child.resolved_layout.flex_grow.unwrap() != 0.0
            {
                let flex_ratio = child.resolved_layout.flex_grow.unwrap() / unresolved_flex_grow;
                let size = self_main_remaining * flex_ratio;
                *child.layout_result.size.main_mut(dir) =
                    Dimension::Px(size) - child_margin.main_total(dir);
                if final_pass {
                    child.layout_result.main_resolved = true;
                } else {
                    current_main_remaining -= size + f64::from(child_margin.main_total(dir));
                }
                ...
                && !child.resolved_layout.overlay
            {
                ...
                // Not included in flex calculations
                if child.resolved_layout.flex_grow.unwrap() == 0.0 {
                    self_main_remaining -= f64::from(
                        child.layout_result.size.main(dir) + child_margin.main_total(dir),
                    );
                }
            }
    ...
    fn resolve_size(
        ...
        if !size.width.resolved() || f64::from(size.width) < 0.0 {
            if available_size.width.resolved() && self.resolved_layout.size.width.is_pct() {
                size.width = size.width.maybe_resolve(&available_size.width);
            } else if self.scroll_x().is_none() && children_size.width.resolved() {
                size.width = children_size.width;
            } else if bounds_size.width.resolved() {
                size.width = size
                    .width
                    .maybe_resolve(&bounds_size.width)
        ...
        if !size.height.resolved() || f64::from(size.height) < 0.0 {
            if available_size.height.resolved() && self.resolved_layout.size.height.is_pct() {
                size.height = size.height.maybe_resolve(&available_size.height);
            } else if self.scroll_y().is_none() && children_size.height.resolved() {
                size.height = children_size.height;
            } else if bounds_size.height.resolved() {
                size.height = size
                    .height
                    .maybe_resolve(&bounds_size.height)
    ...
    fn resolve_layout(
        &mut self,
        // The size of the next available ancenstor constraints, less any padding
        bounds_size: Size,
        // The size of the next available ancestor constraints, less any space taken up by siblings
        available_size: Size,
        caches: &Caches,
        scale_factor: f32,
        final_pass: bool,
        ...
        let available_size = available_size.min(bounds_size);
    ...
    pub(crate) fn calculate_layout(&mut self, caches: &Caches, scale_factor: f32) {
        #[cfg(test)]
        {
            self.resolve_layouts_recursive();
        }
...
mod tests {
    ...
    impl Component for FillBoundser {
        fn fill_bounds(
            ...
            (
                Some(self.size.min(max_width.unwrap_or(f32::INFINITY))),
                Some(self.size.min(max_height.unwrap_or(f32::INFINITY))),
            )
        }
    }
    ...
    #[test]
    fn test_fill_bounds_with_stretch_parent() {
        let mut nodes = node!(Div::new(), [size: [300.0, 300.0]]).push(
            node!(Div::new(), [debug: "container", size_pct: [100.0]]).push(
                node!(Div::new(), [debug: "container2", size_pct: [100.0]]).push(
                    ...
                    node!(Div::new(), [debug: "fake_button", axis_alignment: Stretch, cross_alignment: Stretch]).push(
                        node!(Div::new(), [debug: "fake_button_div"])
    ...
    #[test]
    fn test_flex_grow_with_different_weights_and_container() {
        // Same as previous, but with an additional 100% container, and a Row direction.
        let mut nodes = node!(Div::new(), [size: [420.0, 320.0]]).push(
            node!(
                Div::new(),
                [size_pct: [100.0], margin: [10.0], direction: Direction::Row, axis_alignment: Alignment::Stretch, debug: "container"]
            )
            .push(node!(
                FillBoundser::new(),
    ...
    #[test]
    fn pct_with_fill_bounds_ancestor() {
        ...
        for i in 0..4 {
            ...
            let fill_bounds = &slot_child.children[0];
            assert_eq!(
    ...
    #[test]
    fn test_center_alignment_with_uneven_margins() {
        let mut nodes = node!(
            Div::new(),
            [
                size: [300.0, 200.0],
        ...
        .push(node!(
            ...
            lay!(
                ...
                margin: bounds!(10.0, 30.0, 20.0, 40.0)
            )
        ));
        ...
        assert_eq!(nodes.layout_result.size, size!(300.0, 200.0));
        assert_eq!(nodes.children[0].layout_result.position.left, px!(95.0));
        assert_eq!(nodes.children[0].layout_result.position.top, px!(70.0));
        assert_eq!(nodes.children[0].layout_result.position.right, px!(195.0));
        assert_eq!(nodes.children[0].layout_result.position.bottom, px!(120.0));
    }

    #[test]
    fn test_center_alignment_with_uneven_padding() {
        let mut nodes = node!(
            Div::new(),
            [
                size: [300.0, 200.0],
                direction: Row,
                padding: [10.0, 30.0, 20.0, 40.0],
                axis_alignment: Center,
                cross_alignment: Center
            ]
        )
    ...
    #[test]
    fn test_center_alignment_with_children_of_different_sizes() {
        let mut nodes = node!(
            Div::new(),
            [
                size: [300.0, 200.0],
        ...
        assert_eq!(nodes.layout_result.size, size!(300.0, 200.0));
        let child0 = &nodes.children[0];
        let child1 = &nodes.children[1];
        assert_eq!(child0.layout_result.position.left, px!(100.0));
        assert_eq!(child1.layout_result.position.left, px!(50.0));
    }

    #[test]
    fn test_center_alignment_with_auto_child() {
        let mut nodes = node!(
            Div::new(),
            [
                size: [300.0, 200.0],
        ...
        assert_eq!(nodes.layout_result.size, size!(300.0, 200.0));
        let child = &nodes.children[0];
        assert_eq!(child.layout_result.size, size!(100.0));
        assert_eq!(child.layout_result.position.left, px!(100.0));
        assert_eq!(child.layout_result.position.top, px!(50.0));
    }

    #[test]
    fn test_center_cross_alignment_with_auto_child_in_scrollable() {
        let mut nodes = node!(
            Div::new(),
            [
                size: [300.0, 200.0],
        ...
        .push(
            ...
            [
                size_pct: [100.0], direction: Column, debug: "scrollable",
                cross_alignment: Center
            ])
            // This should be 100px by 100px
            .push(node!(Div::new()).push(node!(FillBoundser::new()))),
        ...
        let child = &scrollable.children[0];
        assert_eq!(child.layout_result.size, size!(100.0));
        assert_eq!(child.layout_result.position.left, px!(100.0));
    }
